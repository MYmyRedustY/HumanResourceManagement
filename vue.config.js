...
if (isProd) {
  ...
  cdn = {
    ...
    js: [
      'https://cdn.jsdelivr.net/npm/vue@2.7.9/dist/vue.js', // vuejs
...
module.exports = {
  ...
  configureWebpack: {
    ...
    // 要排除的包名
    // key(是要排除的包名):value(实际上是实际引入的包的全局变量的变量名)
    // 因为要排除的element-ui，所以之后要引入CDN文件
    // external首先会排除掉定义的包名，空出来的位置会用变量来代替
    externals: externals
  },
  chainWebpack(config) {
